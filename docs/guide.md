...
- **Не гарантирует полноту** по таблице требований. Сейчас закрыты технические поломки Excel и равенства: баланс, касса, нераспределённая прибыль, выручка vs объём×цена, валовая прибыль, состав EBITDA, EBIT, амортизация P&L vs add-back в ОДДС, график ОС, косвенный CFO, график долга, проценты vs ставка, налог vs ставка, дивиденды vs капитал; двойной учёт статьи в двух SUM; колонки сценария в равенствах на дату; live-итоги с разных scenario-листов; стресс режет выручку при той же ставке; одна ставка в двух местах с разными значениями; два курса FX с разными значениями; одна статья в тыс. и в млн.; точка безубыточности по Q×P − COGS vs OPEX; запас по ковенанту, если строка mapped; отрицательный mapped NPV; IRR ниже mapped WACC; объём растёт при плоском CAPEX; объём растёт при плоской себестоимости; объём растёт при плоской дебиторке; объём растёт при плоских запасах; объём растёт при плоской кредиторке; объём растёт при плоской численности.
...
Сервис ищет блоки: колонка названий статей слева, ось времени сверху. Заголовок периода может занимать несколько рядов: даты (`01.07.2022` или Excel-serial с форматом даты), год, отдельно «1 кв.» / месяц. Шаг колонок задаёт зерно ключа (`2022Q3` или `2025-01`). Баннер «Прогноз» или «Факт» над колонками задаёт роль, пока в ячейке нет своего суффикса `E`/`факт`. Имя листа («P&L») — слабый признак.
...
| Двойной СУММ | Одна статья входит в два итога (subtotal + parent SUM) |
| Сценарии | Live P&L ссылается на Base, долг — на Upside |
...
| Касса (I3a) | Остаток ≠ начало + net CF ± mapped FX; касса BS ≠ касса ОДДС |
| Нераспр. прибыль (I3b) | Начало + прибыль − дивиденды ± mapped корректировка ≠ конец |
| Выручка (I4) | P&L ≠ объём × цена |
| Валовая прибыль (I5) | GP ≠ выручка − COGS |
| EBITDA (I6) | EBITDA ≠ выручка − COGS − OPEX |
| EBIT (I7) | EBIT ≠ EBITDA − D&A |
| Амортизация (I8) | D&A в P&L ≠ add-back в ОДДС |
| График ОС (I8b) | Начало + capex − D&A ± mapped FX ≠ конец; PPE BS ≠ график |
| CFO (I9) | NI + D&A − ΔAR − Δзапасы + ΔКЗ ≠ CFO |
| Долг (I10) | Начало + выборка − погашение ± mapped FX ≠ конец; долг BS ≠ график |
| Проценты (I11) | Проценты ≠ ставка × средний долг (месяц: купон / 12) |
| Налог (I12) | Налог + mapped отложенный ≠ ставка × прибыль до налога |
| Дивиденды (I13) | Капитал ≠ начало + прибыль − дивиденды ± mapped эмиссия ± mapped FX |
| Стресс-ставка | Downside снижает выручку, ставка как в Base |
| Конфликт ставки | Налог 20% на Inputs и 24% на P&L |
| Конфликт курса | Курс 80 на Inputs и 85 на P&L |
| Единицы | Выручка 100 и 100 000 на одну дату |
| Безубыточность | Q×P − COGS меньше OPEX |
| NPV | Отрицательный NPV в модели (строка mapped) |
| IRR vs WACC | IRR ниже ставки дисконта из модели |
| Объём vs CAPEX | Объём +10% и больше, CAPEX не растёт |
| Объём vs COGS | Объём +10% и больше, себестоимость не растёт |
| Объём vs ДЗ | Объём +10% и больше, дебиторка не растёт |
| Объём vs запасы | Объём +10% и больше, запасы не растут |
| Объём vs КЗ | Объём +10% и больше, кредиторка не растёт |
| Объём vs штат | Объём +10% и больше, численность не растёт |

Два независимых блока (например Base и Upside) **не склеиваются** в одну проверку баланса. Колонки сценария входят в равенства на дату (баланс, GP, EBIT, налог…); rollforward (касса, долг, ОС) их не считает соседним годом. Колонка «итого» не входит.

### 8. Financial Risk Screen, след и карточка

После `check` код собирает матрицу F01–F14 (касса, долг, дивиденды, runway…), затем `lineage` строит след и только после этого `explain` режет документы. Это **итог** в `GET /report`, не список всех Excel-ошибок.

### 8b. След и карточка (lineage + explain)

От найденной ячейки сервис идёт по графу **к итоговым показателям** и пишет, на что это влияет. Текст integrity-карточки шаблонный. Если есть LLM и есть flagged FRS-issue — модель может переписать cause/impact (текст, не числа), **не выдумывая новые ячейки, цифры, статус F-строки и вердикт**. Находка без ссылки на ячейку из разобранной книги в `/integrity` не попадает.
...
| `degraded` | Вопросов нет, есть eligible flagged FRS-issues после фильтрации, но ChatPort не ответил | да |
...
Дальше: `POST /v1/audits` с файлом и заголовком `X-Actor-Id` → опрос `GET /v1/audits/{id}` раз в 1–2 с → `GET .../report` (итог FRS). Техника и равенства — `GET .../integrity` (карточки `f_*`; итог их не копирует). Карта периодов — `.../layout`, состав статей — `.../mapping`. Пока `running`, в `content` только уже лежащие layout/mapping. Отдельного `/findings` нет. Подробные тела и коды — [api.md](architecture/api.md). Скрипт всех маршрутов: [`scripts/probe/README.md`](../scripts/probe/README.md).
...
**Техника Excel:** ошибки ячеек, циклы, внешние ссылки, VBA/XLM как флаг, разрыв формулы по периодам, константа в формуле, замена формулы числом, неполный `СУММ`, статья в двух `СУММ`, скрытые входы, неиспользуемые ячейки, live-итоги с разных scenario-листов, конфликт ставки в двух местах.

**Финансовые равенства (нужен удачный mapping):**

- I1 — баланс по каждому сломанному периоду, отдельно по блоку модели.
- I3a — касса и **net** денежный поток: mapped `cf.fcf`, иначе derived CFO+CAPEX только если в mapping нет CFF (не один CFO; иначе capex даёт ложный разрыв) ± mapped переоценка кассы. Mapped касса на балансе и в ОДДС — равенство EoP. Нет второй кассы — не выдумываем. Нет FX-строки — курс не выдумываем.
- I3b — нераспределённая прибыль: начало + прибыль − дивиденды ± mapped корректировка НП. Нет корректировки — не выдумываем.
- I4 — выручка ≈ объём × цена (Q×P 950 при P&L 1 020 — error). Без драйверов не спрашивает.
- I5 — валовая прибыль ≈ выручка − COGS.
- I6 — EBITDA ≈ выручка − COGS − OPEX, если все четыре строки найдены.
- I7 — EBIT ≈ EBITDA − D&A.
- I8 — амортизация P&L ≈ add-back в ОДДС (модуль; знак расхода vs add-back не ошибка).
- I8b — график ОС: начало + capex − D&A ± mapped переоценка ОС = конец (capex-отток в ОДДС берётся по модулю). Mapped PPE на балансе и графике — равенство EoP. Нет второй строки — не выдумываем. Нет FX-строки — курс не выдумываем.
- I9 — косвенный CFO: прибыль + D&A ± оборотный капитал. Это equality **check** (`/integrity`), не строка FRS.
- I10 — график долга: начало + выборка − погашение ± mapped переоценка долга = конец. Mapped долг на балансе и графике — равенство EoP. Нет второй строки — не выдумываем. Нет FX-строки — курс не выдумываем.
- I11 — проценты ≈ ставка × средний долг за период; на оси `YYYY-MM` годовой купон делится на 12.
- I12 — (налог + mapped отложенный) ≈ ставка × (NI + налог + отложенный); ставка 25% при эффективных 14% — error. Нет отложенного — не выдумываем.
- I13 — капитал: начало + прибыль − дивиденды + mapped эмиссия ± mapped переоценка капитала = конец. Дивиденды в ОДДС, капитал не падает — error. Нет эмиссии/FX — не выдумываем.

**Сигнал по драйверам:** Q×P − COGS < OPEX — warning, не error. Нет объёма, цены, COGS или OPEX — молчит, выручку вместо Q×P не подставляет.

**Итог (Financial Risk Screen, `GET /report`):** закрытая матрица F01–F14. Нет статьи → «не применимо», не вопрос HITL. F01 отделяет падение/cliff/план-факт от F11 «прогноз лучше истории». F04 рядом показывает NI, CFO, FCF, accruals и код причины; FCF — mapped `cf.fcf` или derived `CFO+CAPEX` со знаком книги. F05 считает DSO от выручки, DIO/DPO только от COGS. F08 требует месячную ось: отрицательная касса — риск, годовая ось или cash plug — недостаточно данных, не ложный позитив. F13 всегда поднимает дивиденды при отрицательном FCFF, а положительный net borrowing повышает приоритет. DSCR/LLCR/PLCR только mapped-строка, порог 1.0x, из CFADS не считаем. F10 без mapped курса и F14 без mapped covenant headroom — «недостаточно данных». `/integrity` — техника и равенства, не свалка в отчёт. F15+ и живая книга Selectel как тест — вне v1.
