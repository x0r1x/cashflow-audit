...
- **Не гарантирует полноту** по таблице требований. Сейчас закрыты технические поломки Excel и равенства: баланс, касса, нераспределённая прибыль, валовая прибыль, EBIT. Двойной учёт OPEX в двух SUM, график долга, налоги, сценарии — в бэклоге, см. [требования](требования.md).
...
| Касса (I3a) | Остаток денег не сходится с потоком |
...
### 8. След и карточка (lineage + explain)
...
```bash
uv sync
cp .env.example .env          # URL моделей по желанию
...
Дальше: `POST /v1/audits` с файлом и заголовком `X-Actor-Id` → опрос `GET /v1/audits/{id}` раз в 1–2 с → `GET .../report`. Подробные тела и коды — [api.md](architecture/api.md). Скрипт всех маршрутов: [`scripts/probe/README.md`](../scripts/probe/README.md).
...
2. **`summary`** — сколько находок по серьёзности и сколько вопросов.
3. **`findings`** — сначала `error`, потом остальное. Откройте `cell_refs` в Excel.
4. **`questions`** — без ответа маппинг (и равенства, которые от него зависят) неполны.
5. **`llm_used` / `embeddings_used`** — вызывалась ли сеть. `false` при шаблонных карточках — норма, если модели не настраивали или находок не было.
...
- I3a — касса и денежный поток (при необходимости с разных листов).
- I3b — нераспределённая прибыль, дивиденды если строка найдена.
- I5 — валовая прибыль ≈ выручка − COGS.
- I7 — EBIT ≈ EBITDA − D&A.

**Сигналы риска** (модель может быть посчитана верно, `severity=risk`): падение выручки или EBITDA ≥ 10% г/г; маржа EBITDA −5 п.п.; отрицательная EBITDA/прибыль два периода подряд; CFO < 0; касса < 0; рост долг/EBITDA на 1.0x и больше (валовый долг, не net); покрытие процентов EBIT/проценты ниже 1.5; прогнозный рост выручки выше истории на 15 п.п.; дивиденды больше CFO. Нет строки в маппинге — сигнал молчит (без HITL). FCF и DSCR не считаем без понятий в онтологии.

**Ещё нет отдельных детекторов:** двойной учёт одной статьи в двух SUM, график погашений, налог vs ставка, знак оборотного капитала, конфликт предпосылок, единицы/валюта, сценарный переключатель, дни ДЗ.
