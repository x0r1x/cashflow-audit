...
**Итог (Financial Risk Screen, `GET /report`):** закрытая матрица F01–F14. Нет статьи → «не применимо», не вопрос HITL. FCF: строка `cf.fcf` или выручка из CFO+CAPEX. DSCR/LLCR только mapped-строка, порог 1.0x, PLCR не выдумываем. F10: нет mapped курса → «недостаточно данных»; курс есть → clear, пару USD/RUB не выдумываем. F14: нет mapped запаса по ковенанту → «недостаточно данных»; запас ≤ 0 → флаг; 3.5x не подставляем. `/integrity` — техника и равенства, не свалка в отчёт.
