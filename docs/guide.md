...
| Касса (I3a) | Остаток ≠ начало + net CF ± mapped FX |
...
- I3a — касса и **net** денежный поток (не один CFO; иначе capex даёт ложный разрыв) ± mapped переоценка кассы. Нет FX-строки — курс не выдумываем. При необходимости с разных листов.
