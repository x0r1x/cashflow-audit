...
3. **`conclusions`** — сводка: сначала недостоверные метрики (`trust`), затем склейка «техника + риск» (`combo`), затем сигналы динамики без поломки расчёта (`dynamics`). У каждого вывода есть `finding_ids` и те же ячейки, что у карточек.
4. **`findings`** — сначала `error`, потом остальное. Откройте `cell_refs` в Excel.
