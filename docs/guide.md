...
3. **`risk_screen`** — матрица F01–F14. `verdict.ready_for_credit` бывает только `false` или `null`.
4. **`issues` / `positives`** — flagged FRS (`B-F*`) и шаблонные плюсы по clear-контролям с числами (маржа, ND/EBITDA, ICR, min cash). `insufficient` в positives не попадает.
5. **`conclusions`** — сводка: сначала недостоверные метрики (`trust`), затем склейка «техника + риск» (`combo`), затем сигналы динамики. `finding_ids` цитируют `f_*` из `/integrity` и/или `B-F*` из `issues`. Карточки Excel/identity сюда целиком не копируются.
...
**Итог (Financial Risk Screen, `GET /report`):** закрытая матрица F01–F14. Нет статьи → «не применимо», не вопрос HITL. F01: падение выручки; mapped объём и цена → декомпозиция; mapped курс → `fx_change`, пару USD/RUB не выдумываем. FCF: строка `cf.fcf` или derived CFO+CAPEX. DSCR/LLCR/PLCR только mapped-строка, порог 1.0x, из CFADS не считаем. F10: нет mapped курса → «недостаточно данных»; курс есть → clear, пару USD/RUB не выдумываем. F14: нет mapped запаса по ковенанту → «недостаточно данных»; запас ≤ 0 → флаг; 3.5x не подставляем. `/integrity` — техника и равенства, не свалка в отчёт. F15+ и живая книга Selectel как тест — вне v1.
