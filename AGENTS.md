...
- **frs:** закрытая матрица F01–F14 всегда; нет concept → `not_applicable` / `insufficient`, не HITL. F01 падение/cliff/план-факт отдельно от F11 «прогноз лучше истории». FCF: mapped `cf.fcf` или derived `CFO+CAPEX` (capex как в книге, без двойного минуса). F05: DSO от revenue, DIO/DPO только от COGS. F08: только год или cash plug без отрицательной кассы → insufficient, не positive/issue. F13: дивиденды при FCFF < 0 всегда flagged; high при net borrowing > 0, иначе medium. DSCR/LLCR/PLCR только mapped `cov.dscr`/`cov.llcr`/`cov.plcr` < 1.0, не считаем из CFADS. F10: нет mapped `fx.rate` → insufficient, не HITL и не USD/RUB; курс есть → clear. F14: нет mapped `covenant.headroom` → insufficient, не HITL и не 3.5x; запас ≤ 0 → flagged. `ready_for_credit` ∈ {false, null}, никогда true. LLM не ставит статус F-строки.
