...
- **frs:** закрытая матрица F01–F14 всегда; нет concept → `not_applicable` / `insufficient`, не HITL. FCF: mapped `cf.fcf` или derived `CFO+CAPEX` (capex как в книге, без двойного минуса). DSCR/LLCR только mapped `cov.dscr`/`cov.llcr` < 1.0, не считаем и не PLCR. F10: нет mapped `fx.rate` → insufficient, не HITL и не USD/RUB; курс есть → clear. F14: нет mapped `covenant.headroom` → insufficient, не HITL и не 3.5x; запас ≤ 0 → flagged. `ready_for_credit` ∈ {false, null}, никогда true. LLM не ставит статус F-строки.
