...
`failed` > `needs_input` (questions непусты) > `degraded` (есть eligible flagged FRS-issues после drop/top-N, ChatPort не ответил, questions пусто) > `succeeded`
...
- **layout:** роль периода ≠ роль статьи. Имя листа — слабый признак. Check-row — метка, не находка. Ось — полоса заголовков (даты, год, квартал/месяц на разных рядах), не одна ячейка. Месяц (`янв.25`, `Jan-25`, `2025-01`, Excel-дата) → `period_key=YYYY-MM`; квартальный шаг оси → `YYYYQn`. Зерно по каденсу всей оси, смешанный шаг не угадываем. `cached_value` serial не декодировать в parquet; layout читает дату только при date-like `number_format` и `WorkbookMeta.date1904`.
...
- **check:** дедуп `(detector, frozenset(cell_refs))`. `agg_double_count` — статья в двух SUM одного столбца; check-row не второй агрегат. `scenario_switch` — live-итоги тянут разные scenario-листы; полные копии Base/Upside не флаг. `stress_rate_unchanged` — Downside режет выручку, mapped ставка не выше Base; нет ставки — молчит, не 20%. `conflicting_rate` — два mapped tax/interest rate на один период с разными значениями; 20 vs 0.20 не конфликт. `conflicting_fx` — два mapped `fx.rate` на один период; 80 vs 0.80 конфликт, не `_as_rate`; USD/RUB не выдумываем. `scale_mismatch` — две mapped статьи одного concept, отношение ≈1000 или ≈1e6; ставки/курс не трогаем. `below_breakeven` — Q×P − COGS < OPEX если volume, price, cogs, opex mapped; нет драйвера — молчит, не HITL и не выручка вместо Q×P. `negative_npv` — mapped `val.npv` < 0 → warning; нет строки — молчит, не HITL, не считает NPV из FCF и не 12% WACC. `irr_below_wacc` — mapped IRR < mapped WACC после `_as_rate`; нет WACC — молчит, не HITL и не 12%. `volume_without_capex` — объём ≥10% г/г, \|CAPEX\| не растёт; нет capex — молчит, не HITL и не штат. `volume_without_cogs` — объём ≥10% г/г, \|COGS\| не растёт; нет cogs — молчит, не HITL. `volume_without_wc` — объём ≥10% г/г, \|AR\| не растёт; нет AR — молчит, не HITL, запасы/КЗ/штат не угадываем. `volume_without_inventory` — объём ≥10% г/г, \|запасы\| не растут; нет inventory — молчит, не HITL, КЗ/штат не угадываем. `volume_without_ap` — объём ≥10% г/г, \|КЗ\| не растёт; нет AP — молчит, не HITL, штат не угадываем. `volume_without_headcount` — объём ≥10% г/г, \|численность\| не растёт; нет headcount — молчит, не HITL, штат не выдумываем. IdentityResolver: нет concept → Question, не угадывать. Периоды по `period_key`, не по номеру колонки. Snapshot-равенства (I1, I4, I5, I6, I7, I8, I12) включают колонки `scenario`; rollforward не склеивает сценарий как соседний год; `total` нигде. I4 — revenue ≈ volume × price; выручка+объём без цены → Question, одна выручка без драйверов молчит. I6 — EBITDA ≈ rev − COGS − OPEX если все четыре mapped; нет OPEX — молчит. I3a — rollforward от net CF: mapped `cf.fcf`, иначе derived `CFO+CAPEX` только если в mapping нет CFF (draw/repay/div/equity_issue); есть CFF без `cf.fcf` → не Finding (не `+CFO`); mapped касса на двух листах → EoP; нет второй кассы/FX — не выдумываем. I3b — opening RE + NI − div + mapped bs.re_adj = closing; нет корректировки — не выдумываем. I9 — CFO ≈ NI + D&A − ΔAR − ΔInv + ΔAP. I10 — opening + drawdown − repayment + mapped fx.debt = closing; нет долга или нет потоков → Question, нет FX — не выдумываем; mapped долг на двух листах → EoP, нет второй строки — не выдумываем. I11 — interest ≈ rate × средний долг; YYYY-MM → купон / 12; нет ставки → Question. I12 — (tax + mapped pnl.deferred_tax) ≈ rate × (NI + tax + deferred); нет ставки/налога/NI → Question, не 20%, нет deferred — не выдумываем. I13 — opening equity + NI − div + mapped cf.equity_issue + mapped fx.equity = closing; дивиденды без капитала → Question, нет эмиссии/FX — не выдумываем. I8 — abs(P&L D&A) ≈ abs(CF add-back); одна сторона без другой → Question. I8b — opening + abs(capex) − abs(DA) + mapped fx.ppe = closing по PPE; нет графика или нет потоков → Question, нет FX — не выдумываем; mapped PPE на двух листах → EoP, нет второй строки — не выдумываем. `cell_refs` всех сторон равенства. `risk.*` здесь нет (это frs).
- **frs:** закрытая матрица F01–F14 всегда; нет concept → `not_applicable` / `insufficient`, не HITL. F01 падение/cliff/план-факт отдельно от F11 «прогноз лучше истории». FCF: mapped `cf.fcf` или derived `CFO+CAPEX` (capex как в книге, без двойного минуса). F05: DSO от revenue, DIO/DPO только от COGS. F08: только год или только квартал или cash plug без отрицательной кассы → insufficient, не positive/issue. F13: дивиденды при FCFF < 0 всегда flagged; high при net borrowing > 0, иначе medium. DSCR/LLCR/PLCR только mapped `cov.dscr`/`cov.llcr`/`cov.plcr` < 1.0, не считаем из CFADS. F10: нет mapped `fx.rate` → insufficient, не HITL и не USD/RUB; курс есть → clear. F14: нет mapped `covenant.headroom` → insufficient, не HITL и не 3.5x; запас ≤ 0 → flagged. `ready_for_credit` ∈ {false, null}, никогда true. LLM не ставит статус F-строки.
- **explain:** `integrity.json` — техника/identity (шаблон, без LLM). Тонкий `report.json`: `risk_screen {matrix, issues, positives, verdict}`, conclusions (цитаты `f_*` и/или `B-F*`), индекс questions; `findings` пуст. `ready_for_credit` только в `risk_screen.verdict`. LLM только проза flagged issues (`cause`/`impact`), не статус F-строки и не вердикт; числа и refs не меняет. Drop без ref ∈ IR. SeverityPolicy: freeze (`hist_manual_adjustment`, `edge_period`, `likely_intentional`) не выше `warning`.
...
| layout | две оси на листе = два блока; hidden колонка не label; роль периода ≠ роли статьи; check-row не Finding; `янв.25` → `2025-01`; многострочная ось (дата/год/`N кв.`) → `YYYYQn`; serial+date format → период; баннер Прогноз/Факт наследуется |
...
| frs | матрица всегда 14 строк; пустой mapping → 14× NA, 0 issues; F04/F13 без FCF → insufficient; F08 год/cash plug → insufficient; DSCR не выдуман |
